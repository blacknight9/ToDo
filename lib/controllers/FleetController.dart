...
class FleetController extends GetxController {
  ...
  RxList<ResModel> reservations =
      <ResModel>[].obs; // Using RxList for reactivity
  ...
  Future<void> selectTime(BuildContext context) async {
    ...
    if (pickedTime != null) {
      ...
      print(
          'Selected DateTime: $selectedDateTime'); // Prints the selected date with time.
    }
  }
  ...
  searchRes() async {
    if (resNumController.text.isNotEmpty) {
      try {
        var getRes = await CollectionRef.path(path: 'reservations')
            .doc(resNumController.text)
            .get();
  ...
  void fetchVans() async {
    FirebaseFirestore.instance
        ...
        .listen((snapshot) {
      var vans =
          snapshot.docs.map((doc) => AddVanModel.fromMap(doc.data())).toList();
    ...
    if (key.currentState!.validate() && packUnitController.text.isNotEmpty) {
      await CollectionRef.path(path: 'packers')
        ...
        if (value.exists) {
          ...
          try {
            CollectionRef.path(path: 'packers')
                .doc(packUnitController.text)
                .set(
                  AddVanModel(
 ...
 Future<void> addRes(
    ...
    if (key.currentState!.validate() &&
      ...
      try {
          isEdit == false
              ? await CollectionRef.path(path: 'reservations')
                  .doc(resNumber)
                  .set(ResModel(
                    resNumber: resNumber,
                    ...
                    isAllDay: true,
                    unit: selectedVan.value != null
                        ? selectedVan.value!.unitNumber
                        : unitController.text,
                    size: selectedVan.value != null
                        ? selectedVan.value!.seats
                        : sizeController.text,
